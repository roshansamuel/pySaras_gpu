...
nu, kappa = cp.sqrt(Pr/Ra), 1.0/cp.sqrt(Ra*Pr)
...
x = cp.linspace(0, Lx + hx, Nx + 2, endpoint=True) - hx/2
x = x.get()
y = cp.linspace(0, Ly + hx, Ny + 2, endpoint=True) - hy/2
y = y.get()
z = cp.linspace(0, Lz + hx, Nz + 2, endpoint=True) - hz/2
z = z.get()
...
if restart == 1:
    ...
    U, V, W, P, T = cp.asarray(Vx), cp.asarray(Vy), cp.asarray(Vz), cp.asarray(Press), cp.asarray(Temp)
    ...
    P = cp.zeros([Nx+2, Ny+2, Nz+2])

    T = cp.zeros([Nx+2, Ny+2, Nz+2])

    #T[1:-1, 1:-1, 1:-1] = (1.0 - 0.5/(Nz-1)) - z[1:Nz-1]

    #print(T[5, 5, 1:Nz-1])

    U = cp.zeros([Nx+2, Ny+2, Nz+2]) #cp.random.rand(Nx, Ny, Nz) #

    V = cp.zeros([Nx+2, Ny+2, Nz+2]) #cp.random.rand(Nx, Ny, Nz) #

    W = cp.zeros([Nx+2, Ny+2, Nz+2]) #cp.random.rand(Nx, Ny, Nz) #
...
def getDiv(U, V, W):
    ...
    maxDiv = cp.max(abs(((U[2:, 1:-1, 1:-1] - U[:-2, 1:-1, 1:-1])*0.5/hx +
                                (V[1:-1, 2:, 1:-1] - V[1:-1, :-2, 1:-1])*0.5/hy +
                                (W[1:-1, 1:-1, 2:] - W[1:-1, 1:-1, :-2])*0.5/hz)))
    
    #return cp.unravel_index(divNyat.argmax(), divMat.shape), cp.mean(divMat)
    return maxDiv
...
def uJacobi(rho):
    ...
    while True:
        ...
        maxErr = cp.amax(cp.abs(rho[1:-1, 1:-1, 1:-1] - (U[1:-1, 1:-1, 1:-1] - 0.5*nu*dt*(
...
def vJacobi(rho):
    ...
    while True:
        ...
        maxErr = cp.amax(cp.abs(rho[1:-1, 1:-1, 1:-1] - (V[1:-1, 1:-1, 1:-1] - 0.5*nu*dt*(
...
def wJacobi(rho):    
    ...
    while True:
        ...
        maxErr = cp.amax(cp.abs(rho[1:-1, 1:-1, 1:-1] - (W[1:-1, 1:-1, 1:-1] - 0.5*nu*dt*(
...
def TJacobi(rho):
    ...
    while True:
        ...
        maxErr = cp.amax(cp.abs(rho[1:-1, 1:-1, 1:-1] - (T[1:-1, 1:-1, 1:-1] - 0.5*kappa*dt*(
...
def Poisson_Jacobi(rho):   
    ...
    while True:
        ...
        maxErr = cp.amax(cp.abs(rho[1:-1, 1:-1, 1:-1] -((
...
while True:
    ...
    dtnew = Cn/cp.amax((abs(U)/hx) + (abs(V)/hy) + (abs(W)/hz))
    #dtnew = Cn/max((cp.amax(cp.abs(U))/hx), (cp.amax(cp.abs(V))/hy), (cp.amax(cp.abs(W))/hz))
    ...
    rData[0][1:-1, 1:-1, 1:-1] = U[1:-1, 1:-1, 1:-1] + dt*(rData[0][1:-1, 1:-1, 1:-1] - cp.sqrt((Ta*Pr)/Ra)*(-V[1:-1, 1:-1, 1:-1]) - (P[2:, 1:-1, 1:-1] - P[:-2, 1:-1, 1:-1])/(2.0*hx))
    uJacobi(rData[0])

    rData[0] = computeNLinDiff_Y(U, V, W)
    rData[0][1:-1, 1:-1, 1:-1] = V[1:-1, 1:-1, 1:-1] + dt*(rData[0][1:-1, 1:-1, 1:-1] - cp.sqrt((Ta*Pr)/Ra)*(U[1:-1, 1:-1, 1:-1]) - (P[1:-1, 2:, 1:-1] - P[1:-1, :-2, 1:-1])/(2.0*hy))
    vJacobi(rData[0])
